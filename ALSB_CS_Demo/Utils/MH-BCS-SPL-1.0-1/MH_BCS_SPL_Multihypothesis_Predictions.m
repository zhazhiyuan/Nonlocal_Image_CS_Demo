% function predicted_image = MH_BCS_SPL_Multihypothesis_Predictions(Y, ...
%     Phi, reference,block_size,match_size,w,test_lambda)
% 
% MH_BCS_SPL_Multihypothesis_Predictions is a function to generate
% multiple hypotheses for a block from spatially surrounding blocks. 
...
% 
% [Notice]
% (1) Tikhonov regularization parameter: lambda
    ...
    for j = 1:block_size:num_cols
        ...
        if match_size ~= 1
           ...
           % are used in generation of multiple hypotheses
            test_lambda = adaptive_lambda(cur_proj, A);
        end
        
        G = diag(norms);    % diagonal Tikhonov matrix
        weights = (A'*A + test_lambda.*G)\(A'*cur_proj); 
